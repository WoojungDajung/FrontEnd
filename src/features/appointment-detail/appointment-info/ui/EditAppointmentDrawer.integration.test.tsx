...
// EditAppointmentForm: 폼 로직은 EditAppointmentForm.integration.test.tsx에서 검증.
// 여기서는 onSubmitSuccess 연결 확인만 하므로 트리거 버튼으로 대체한다.
jest.mock("./EditAppointmentForm", () => ({
  __esModule: true,
...
// LoadingSpinner: 삭제 로딩 오버레이 (동작 검증 불필요)
...
function setupFetch(deleteOk = true) {
...
describe("EditAppointmentDrawer 통합", () => {
  ...
  describe("약속 없애기 버튼 표시 여부", () => {
    it("isHost=true이면 '약속 없애기' 버튼이 표시된다", () => {
      renderWithProviders(<EditAppointmentDrawer {...DEFAULT_PROPS} isHost={true} />);

      expect(
        screen.getByRole("button", { name: "약속 없애기" }),
      ).toBeInTheDocument();
    });

    it("isHost=false이면 '약속 없애기' 버튼이 표시되지 않는다", () => {
      renderWithProviders(<EditAppointmentDrawer {...DEFAULT_PROPS} isHost={false} />);

      expect(
        screen.queryByRole("button", { name: "약속 없애기" }),
      ).not.toBeInTheDocument();
    });
  });

  describe("약속 없애기 — confirm 확인", () => {
    it("DELETE API가 호출된다", async () => {
      renderWithProviders(<EditAppointmentDrawer {...DEFAULT_PROPS} />);

      await user.click(screen.getByRole("button", { name: "약속 없애기" }));

      await waitFor(() =>
        expect(mockFetch).toHaveBeenCalledWith(
          expect.stringContaining("appointment/appt-1"),
    ...
    it("삭제 성공 시 약속 목록 페이지로 이동한다", async () => {
      renderWithProviders(<EditAppointmentDrawer {...DEFAULT_PROPS} />);
  ...
  describe("약속 없애기 실패", () => {
    ...
    it("삭제 실패 시 router.push는 호출되지 않는다", async () => {
      renderWithProviders(<EditAppointmentDrawer {...DEFAULT_PROPS} />);
  ...
  describe("폼 저장 성공 후 drawer 닫힘", () => {
    it("저장 성공 시 setOpen(false)가 호출된다", async () => {
      ...
      renderWithProviders(
        <EditAppointmentDrawer {...DEFAULT_PROPS} setOpen={setOpen} />,
      );
